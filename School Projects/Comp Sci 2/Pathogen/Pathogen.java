...
public class Pathogen
{
	...
	public static HashSet<String> findPaths(char[][] maze)
	{
		...
		return findPaths(maze, visited, height, width,
						 path, pathSet, startRow, startCol);
		
	}

	private static HashSet<String> findPaths(char[][] maze, char[][] visited, int height, int width,
						 StringBuilder path, HashSet<String> pathSet, int currentRow, int currentCol)
	{
		int exitFlag = 0;
		// This conditional block prints the maze when a new move is made.
		if (Pathogen.animationEnabled)
		...
		for (int i = 0; i < moves.length; i++)
		{
			...
			// Perform recursive descent.
			pathSet = findPaths(maze, visited, height, width, path, pathSet, newRow, newCol);
	...
	// that position before, and it's not a wall amd the move is within bounds).
	...
	public static void main(String [] args) throws IOException
	{
		...
		*/
		HashSet<String> paths = findPaths(maze);
		for(String s : paths)
		{
