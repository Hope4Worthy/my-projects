...
if [ ! -f assignment03.c ]; then
	echo ""
	echo " Error: You must place assignment03.c in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f assignment03.h ]; then
	echo ""
	echo " Error: You must place assignment03.h in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
...
do
	if [ "$i" = "11" ] || [ "$i" = "12" ] || [ "$i" = "13" ]; then
		...
		# Most functions have three test cases.
		check_test_case "$i"a
		check_test_case "$i"b
		check_test_case "$i"c
	fi
...
grep -s -q "^#include \"assignment03\.h\"" assignment03.c
grep_val=$?

if [[ $grep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your assignment03.c file does not appear to have a proper"
	echo "  #include statement for assignment03.h. Please read Section 1 of the"
...
run_test_case()
{
	...
	# Attempt to compile.
	gcc assignment03.c $testcase_file 2> /dev/null
	compile_val=$?
...
echo ""
echo "  Currently passing $PASS_CNT/$TOTAL_TEST_CNT test cases."
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " assignment03.c | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" assignment03.c | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" assignment03.c | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" assignment03.c -n"
		echo "     grep \"\$(printf '^\t')\" assignment03.c -n"
