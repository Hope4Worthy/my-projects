...
TOTAL_TEST_CNT=`expr $NUM_TEST_CASES + 2`
...
if [ ! -f assignment02.c ]; then
	echo ""
	echo " Error: You must place assignment02.c in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f assignment02.h ]; then
	echo ""
	echo " Error: You must place assignment02.h in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
...
grep -s -q "^#include \"assignment02\.h\"" assignment02.c
grep_val=$?

if [[ $grep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your assignment02.c file does not appear to have a proper"
	echo "  #include statement for assignment02.h. Please read Section 1 of the"
...
run_test_case()
{
	...
	# Attempt to compile.
	gcc assignment02.c $testcase_file 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " assignment02.c | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" assignment02.c | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" assignment02.c | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" assignment02.c -n"
		echo "     grep \"\$(printf '^\t')\" assignment02.c -n"
